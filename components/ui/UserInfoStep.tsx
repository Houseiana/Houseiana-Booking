...
export function UserInfoStep({
  ...
  const t = {
    yourInfo: locale === 'ar' ? 'معلوماتك' : 'Your Information',
    infoMessage: locale === 'ar' ? 'نحتاج إلى معلوماتك للمتابعة' : 'We need your information to proceed',
    ...
    whatsappPlaceholder: locale === 'ar' ? '1234 5678' : '1234 5678',
  ...
  const handlePhoneChange = (value: string) => {
    ...
    onWhatsappChange(`${countryCode}${value}`);
  };

  const handleCountryCodeChange = (value: string) => {
    setCountryCode(value);
    onWhatsappChange(`${value}${phoneNumber}`);
  };

  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="mb-4 text-center">
        <h3 className="text-xl font-bold text-gray-900">{t.yourInfo}</h3>
        <p className="mt-1 text-sm text-gray-600">{t.infoMessage}</p>
      </div>
      ...
      <div>
        <label className="mb-2 flex items-center justify-between text-sm font-medium text-gray-700">
          ...
          >
            <Phone size={12} />
            {locale === 'ar' ? 'راسلنا: +974 30424433' : 'Text us: +974 30424433'}
          </a>
        </label>
